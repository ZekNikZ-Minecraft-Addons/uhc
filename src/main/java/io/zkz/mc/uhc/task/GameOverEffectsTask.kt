...
class GameOverEffectsTask : MinigameTask(20, 20) {
    ...
    private val subtitle: Component = if (settingsManager.teamGame.value == TeamStatus.TEAM_GAME) {
        mm("<0> <legacy_aqua>wins!", minigameService.currentRound.aliveTeams.first().displayName)
    } else {
        mm("<legacy_aqua><0> wins!", minigameService.currentRound.onlineAlivePlayers.first())
    }
    ...
    override fun run() {
        ...
        minigameService.currentRound.onlineAlivePlayers.forEach { p ->
